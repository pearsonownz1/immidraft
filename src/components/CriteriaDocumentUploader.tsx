...
export function CriteriaDocumentUploader({
  criterionId,
  criterionTitle,
  documents = [],
  onDocumentsChange,
  ...
  const handleAddDocument = async () => {
    ...
    try {
      ...
      const newDocument: Document = {
        id: crypto.randomUUID(), // Use standard UUID format
      ...
      if (newDocumentFile) {
        ...
        const filePath = `${criterionId}/${newDocument.id}.${fileExt}`;
        ...
        newDocument.file_url = urlData.publicUrl;
      }
  ...
  const handleEditDocument = async () => {
    ...
    try {
      ...
      if (editingDocument.file && editingDocument.file !== documents.find(d => d.id === editingDocument.id)?.file) {
        ...
        // Upload file to Supabase Storage
        const fileExt = editingDocument.fileName?.split('.').pop();
        const filePath = `${criterionId}/${editingDocument.id}.${fileExt}`;
        
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('documents')
          .upload(filePath, editingDocument.file, { upsert: true });
        ...
        updatedDocument.file_url = urlData.publicUrl;
      ...
      if (updatedDocument.processing && updatedDocument.file_url) {
