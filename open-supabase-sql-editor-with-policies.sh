...
# Script to open the Supabase SQL Editor with the storage bucket policies SQL script
...
if command -v pbcopy > /dev/null; then
  # macOS
  cat create-storage-bucket-policies.sql | pbcopy
  echo "SQL script copied to clipboard. Paste it into the SQL Editor."
elif command -v xclip > /dev/null; then
  # Linux with xclip
  cat create-storage-bucket-policies.sql | xclip -selection clipboard
  echo "SQL script copied to clipboard. Paste it into the SQL Editor."
elif command -v clip > /dev/null; then
  # Windows
  cat create-storage-bucket-policies.sql | clip
  echo "SQL script copied to clipboard. Paste it into the SQL Editor."
else
  echo "Could not copy SQL script to clipboard. Please open 'create-storage-bucket-policies.sql' and copy its contents manually."
fi
...
echo "3. Verify that the policies were created by checking the results of the SELECT query at the end"
echo ""
echo "After creating the bucket and policies, redeploy the application to Vercel to verify the fix."
